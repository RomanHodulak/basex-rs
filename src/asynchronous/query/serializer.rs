...
///
/// ```
/// # use basex::asynchronous::{Client, serializer::Options, serializer::ParseError};
/// # use std::str::FromStr;
/// # #[tokio::main(flavor = "current_thread")]
/// # async fn main() -> std::result::Result<(), std::boxed::Box<dyn std::error::Error>> {
/// // Connect to the server
/// let mut client = Client::connect("localhost", 1984, "admin", "admin").await?;
///
...
/// let client = options.save(client).await?;
/// # Ok(())
/// # }
/// ```
...
impl Options {
    ...
    pub async fn save<T: AsyncWriteExt + AsyncReadExt + Unpin>(
        &self,
        client: Client<T>,
    ) -> std::result::Result<Client<T>, ConnectionError> {
        let (client, _) = client
